...
def mlp_policy(x, a, hidden_sizes, activation, output_activation):
    act_dim = a.shape.as_list()[-1]
    net = mlp(x, list(hidden_sizes), activation, activation)
    pi = tf.layers.dense(net, act_dim, activation=tf.tanh)
...
def a2c(x, a, hidden_sizes=(400,300), activation=tf.nn.relu, 
                     output_activation=None, policy=mlp_policy, action_space=None):
    # policy
    with tf.variable_scope('pi'):
        pi = policy(x, a, hidden_sizes, activation, output_activation)
